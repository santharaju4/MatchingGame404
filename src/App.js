...
class App extends Component {
  state = {
    matchImagesList: imagesList[0].imageUrl,
    matchImagesListId: imagesList[0].id,
    isActive: false,
  ...
  renderScoreCard = () => {
    ...
    return (
      <ul className="score-card-list-item">
        <li>
          <div className="score-card-container">
            <div className="score-card">
              ...
              />
              <p className="win-score-title">YOUR SCORE</p>
  ...
  onCheck = () => {
    const {activeImageId, matchImagesListId} = this.state
    console.log(activeImageId)
    const imagesListLength = imagesList.length - 1
    const randomActiveImages = Math.ceil(Math.random() * imagesListLength)
    console.log(randomActiveImages)
    const obj = imagesList[randomActiveImages]

    const randomChangeImageUrlList = obj.imageUrl
    console.log(randomChangeImageUrlList)
    const idRequired = obj.id

    if (activeImageId === matchImagesListId) {
      this.setState(prevState => ({score: prevState.score + 1}))
      this.setState({
        matchImagesList: randomChangeImageUrlList,
        matchImagesListId: idRequired,
      })
    } else {
      clearInterval(this.TimeInterval)
      this.setState({isActive: true})
  ...
  renderGameStart = () => {
    const {matchImagesList, activeTabId} = this.state
    ...
    return (
      <ul>
        <div className="displayContainer">
          <img src={matchImagesList} alt="match" className="selectedImage" />
          <ul>
  ...
  render() {
    const {isActive, score, time} = this.state
              ...
              <div className="navbar-container">
                ...
                <div className="score-time-container">
                  <p className="score-title">
                    Score: <span className="counts">{score}</span>
                  </p>
                  <img
                  ...
                  />
                  <p className="timer-container"> {time} sec</p>
                </div>
              </div>
            </li>
            <ul className="score-card-list-item">
              <div>
                <li>
                  {isActive ? this.renderScoreCard() : this.renderGameStart()}
                </li>
              </div>
            </ul>
          </div>
